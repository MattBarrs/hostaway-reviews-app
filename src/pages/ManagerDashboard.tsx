...
const groupByProperty = (reviews: Review[]) => {
  ...
  return Array.from(map.entries()).map(([listingName, revs]) => {
    const ratings = revs
      .map((r) =>
        r.rating === null || r.rating === undefined ? null : r.rating,
      )
...
const ManagerDashboard: React.FC = () => {
  ...
  const visible = useMemo(() => {
    ...
    if (minRating !== "") {
      list = list.filter(
        (p) => (p.avgFiltered ?? p.avgRating ?? 0) >= Number(minRating),
      );
    }
    if (sort === SortType.RatingDesc)
      list.sort(
        (a, b) =>
          (b.avgFiltered ?? b.avgRating ?? 0) -
          (a.avgFiltered ?? a.avgRating ?? 0),
      );
    ...
    if (sort === SortType.Name)
      list.sort((a, b) => a.listingName.localeCompare(b.listingName));
    return list;
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div>
                        ...
                        {(p.avgFiltered ?? p.avgRating)
                          ? ((p.avgFiltered ?? p.avgRating) as number).toFixed(
                              1,
                            )
                          : "N/A"}
                      </div>
